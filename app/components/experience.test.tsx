...
describe("StoryStage family story flow", () => {
  it("shows six stories and filters the library by category", async () => {
    ...
    stories.forEach((story) => story.roles.forEach((role) => expect(story.lines.filter(({ roleId }) => roleId === role.id).length).toBeGreaterThanOrEqual(5)));
  ...
  it("offers a seventeen-page color learning pack without dropping script lines or cards", () => {
    ...
    expect(pack.querySelectorAll(".memory-card")).toHaveLength(12);
    expect(pack.querySelectorAll(".blank-memory-card")).toHaveLength(2);
    expect(pack.textContent).toContain("我遇到的新词");
