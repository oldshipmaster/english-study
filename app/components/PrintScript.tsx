  ...
  return (
    <section className="print-panel">
      ...
      {printMode === "learning-pack" ? <LearningPackPrint story={story} assignments={assignments} /> : <article className="print-sheet" role="region" aria-label={printLabel}>
        ...
        <div className="print-scenes">
          {scenes.map((lines, sceneIndex) => (
            <section className="print-scene" key={sceneIndex} aria-label={`第 ${sceneIndex + 1} 场`}>
              ...
              {selectedPerson ? <div className="role-rehearsal-check"><strong>本场三遍排练 · 先给自己的每句圈 1–3 个关键词</strong><span>□ 看完整台词</span><span>□ 只看圈词</span><span>□ 不看稿</span><em>先听到上一句再开口 · {sceneIndex + 1} / {scenes.length}</em></div> : null}
            </section>
          ))}
        </div>
        ...
        {selectedPerson ? <section className="role-performance-note"><strong>演后 30 秒 · 两颗星 + 一个愿望</strong><p>日期：________　□ 第 0 天首演 · 看完整台词　□ 第 7 天重演 · 只看圈词</p><p>我的自评：□ 听到提示再开口　□ 声音清楚　□ 说错能从句首重来</p><p>家长：⭐ __________________　⭐ __________________　🌱 下次试试 __________________</p></section> : null}
